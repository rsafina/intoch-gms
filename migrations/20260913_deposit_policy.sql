-- Requires completed Phase 1 roles. Apply to each Supabase project before frontend deploy.
-- Does not rerun or replace the authenticated public RPC wrapper.
begin;
do $$ begin
 if to_regprocedure('app_private.create_public_reservation(text,text,integer,date,time without time zone,text,uuid,text)') is null then
  raise exception 'Apply Phase 1 role enforcement first';
 end if;
end $$;
alter table public.reservations add column if not exists deposit_basis text not null default 'area' check (deposit_basis in ('area','pax'));
alter table public.reservations add column if not exists deposit_quote_required boolean not null default false;
alter table public.reservations add column if not exists deposit_invoice_format text check (deposit_invoice_format in ('simple','detailed','unselected'));
-- NULL format preserves legacy size-based routing. New large bookings explicitly choose.
create or replace function app_private.create_public_reservation(
...
as $function$
  declare
    ...
    v_policy jsonb := coalesce(get_setting('reservation_form'), '{}'::jsonb);
    v_basis text := case when v_policy->>'deposit_basis'='pax' then 'pax' else 'area' end;
    v_free integer := greatest(0, coalesce((v_policy->>'deposit_free_pax')::integer,1));
    ...
    v_max_pax  integer := greatest(coalesce(case when v_basis='pax' then (v_policy->>'deposit_regular_max_pax')::integer else (v_cfg->>'max_pax')::integer end,20),1);
  ...
  begin
    ...
    if v_basis='pax' then
      v_dep_req := p_pax > v_free;
      v_dep_amt := null;
      v_dep_note := case when v_dep_req then 'Guest-count rule: staff will quote deposit' else 'Guest-count rule: no deposit' end;
    end if;
    ...
    if v_status = 'Reserved' and v_dep_req then
      ...
      if v_basis='area' then v_due_at := (p_date + p_time) at time zone 'Asia/Jakarta'; end if;
    end if;
    ...
    insert into reservations
      ...
      deposit_due_at, booking_duration_minutes, is_large_party, deposit_basis, deposit_quote_required, deposit_invoice_format)
    values
      ...
      v_due_at, v_duration, p_pax > v_max_pax, v_basis, v_basis='pax' and v_dep_req, case when p_pax>v_max_pax then 'unselected' else 'simple' end)
    ...
    return jsonb_build_object('ok', true, 'reservation_id', v_res_id,
      ...
      'deposit_expected', v_dep_amt, 'deposit_basis', v_basis, 'awaiting_quote', v_basis='pax' and v_dep_req and v_status='Incoming');
  end;
  $function$;
...
begin
 if old.status='Waitlist' and new.status in ('Reserved','Confirmed')
    ...
    and not coalesce(new.is_large_party,new.pax > coalesce((get_setting('reservation_hours')->>'max_pax')::integer,20)) then
  -- A waiver is an explicit decision, not acceptance with a deposit still due.
  if new.deposit_rule_note is distinct from old.deposit_rule_note and new.deposit_rule_note like 'Waived by %' then return new; end if;
  if new.deposit_basis='pax' and new.deposit_quote_required then
   new.status:='Incoming'; new.deposit_required:=true; new.deposit_due_at:=null;
  elsif new.deposit_basis='area' and coalesce(new.deposit_expected,0)>0 then
   new.status:='Incoming'; new.deposit_required:=true;
   new.deposit_due_at:=(new.reservation_date+new.reservation_time) at time zone 'Asia/Jakarta';
  end if;
 end if;
 return new;
end $$;
revoke all on function public.activate_waitlist_deposit() from public,anon,authenticated;

create or replace function app_private.guard_deposit_policy() returns trigger
language plpgsql security definer set search_path=public,pg_temp as $$
begin
 if tg_op='UPDATE' and new.deposit_invoice_format is distinct from old.deposit_invoice_format
    and exists(select 1 from invoices where reservation_id=new.id and kind='deposit' and status='issued') then
  raise exception 'An invoice already exists. Keep its payment format.';
 end if;
 if new.deposit_basis='pax' then
  if new.status='Incoming' and new.deposit_required then
   new.deposit_due_at:=case when new.deposit_asked_at is not null and coalesce(new.deposit_expected,0)>0
    then (new.reservation_date+new.reservation_time) at time zone 'Asia/Jakarta' else null end;
  elsif new.status='Waitlist' then new.deposit_due_at:=null;
  end if;
 end if;
 return new;
end $$;
drop trigger if exists zz_reservation_deposit_policy on public.reservations;
create trigger zz_reservation_deposit_policy before insert or update on public.reservations
for each row execute function app_private.guard_deposit_policy();

create or replace function public.set_reservation_deposit_request(p_reservation_id uuid,p_amount numeric,p_format text)
returns jsonb language plpgsql security definer set search_path=public,pg_temp as $$
declare booking public.reservations;
begin
 perform app_private.require_access('staff');
 if p_amount is null or round(p_amount)<=0 or p_amount::text in ('NaN','Infinity','-Infinity') or p_format not in ('simple','detailed') or p_format is null then
  raise exception 'Enter a positive deposit amount and choose an invoice format.';
 end if;
 select * into booking from reservations where id=p_reservation_id and deleted_at is null for update;
 if not found or booking.status not in ('Incoming','Waitlist','Reserved','Confirmed') then raise exception 'This booking cannot request a deposit.'; end if;
 if not coalesce(booking.is_large_party,booking.pax > coalesce((get_setting('reservation_hours')->>'max_pax')::integer,20)) and booking.status='Waitlist' then raise exception 'Accept the capacity request before requesting payment.'; end if;
 if not coalesce(booking.is_large_party,booking.pax > coalesce((get_setting('reservation_hours')->>'max_pax')::integer,20)) and p_format<>'simple' then raise exception 'Use a simplified invoice for a regular booking.'; end if;
 if exists(select 1 from invoices where reservation_id=booking.id and kind='deposit' and status='issued') then raise exception 'An invoice already exists. Edit the existing invoice instead.'; end if;
 update reservations set deposit_required=true,deposit_expected=round(p_amount),deposit_invoice_format=p_format,updated_at=now() where id=booking.id;
 return jsonb_build_object('ok',true);
end $$;
revoke all on function public.set_reservation_deposit_request(uuid,numeric,text) from public,anon,authenticated;
grant execute on function public.set_reservation_deposit_request(uuid,numeric,text) to authenticated;

create or replace function app_private.validate_deposit_policy_settings() returns trigger
language plpgsql set search_path=public,pg_temp as $$
declare free_pax integer; regular_pax integer;
begin
 if new.key='reservation_form' and new.value->>'deposit_basis'='pax' then
  free_pax:=coalesce((new.value->>'deposit_free_pax')::integer,1);
  regular_pax:=coalesce((new.value->>'deposit_regular_max_pax')::integer,20);
  if free_pax<0 or regular_pax<1 or free_pax>regular_pax then raise exception 'No-deposit threshold must be between 0 and the regular booking maximum.'; end if;
 end if;
 return new;
end $$;
drop trigger if exists validate_deposit_policy_settings on public.app_settings;
create trigger validate_deposit_policy_settings before insert or update on public.app_settings
for each row execute function app_private.validate_deposit_policy_settings();
