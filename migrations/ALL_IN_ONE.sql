...
--  10. 20260916_finance_role.sql (Finance permissions; redeploy staff-account).
--  11. 20260917_session_notifications.sql (session validity/checklist; redeploy staff-account).
--  12. 20260918_spending_deposit_choice.sql (explicit deposit-inclusive spending).
...
-- ============================================================
-- SETTLEMENT PAYMENTS AND PREPAID COMPLETION (2026-09-08)
-- Saved invoices are billing documents. Payments are recorded separately.
-- Completion counts the full final invoice once (or recorded payments when
-- there is no final invoice), then adds only the extras entered by staff.
-- ============================================================
begin;

alter table public.visits add column if not exists billing_base_amount numeric;
alter table public.visits add column if not exists extra_spend_amount numeric;
comment on column public.visits.billing_base_amount is 'Final invoice/payment base included in spend_amount at completion; never added twice on re-completion.';
comment on column public.visits.extra_spend_amount is 'Extra spending added to billing_base_amount at completion. Separate so reopening does not add the full bill again.';

create or replace function public.record_invoice_payment(
  p_invoice_id uuid, p_payment_id uuid, p_amount numeric, p_paid_on date,
  p_method text default null, p_reference text default null,
  p_note text default null, p_staff_id uuid default null
...
begin
  if p_payment_id is null or p_amount is null or p_amount = 0
    ...
    return jsonb_build_object('ok', false, 'message', 'Enter a non-zero payment amount.');
  end if;
  select reservation_id into reservation_key from public.invoices where id = p_invoice_id;
  if reservation_key is null then
    return jsonb_build_object('ok', false, 'message', 'This invoice is not linked to a reservation.');
  end if;
  -- Shared lock order with completion: reservation first, then invoice/visit.
  perform 1 from public.reservations where id = reservation_key and deleted_at is null for update;
  if not found then
    return jsonb_build_object('ok', false, 'message', 'That reservation no longer exists.');
  end if;
  select * into invoice_row from public.invoices where id = p_invoice_id for update;
  if invoice_row.kind <> 'settlement' or invoice_row.reservation_id is distinct from reservation_key then
    return jsonb_build_object('ok', false, 'message', 'This invoice is not a settlement invoice.');
  end if;
  select * into existing_payment from public.invoice_payments where id = p_payment_id;
  if found then
    if existing_payment.invoice_id is distinct from p_invoice_id or existing_payment.amount is distinct from p_amount then
      return jsonb_build_object('ok', false, 'message', 'This payment request was already used. Reopen Record payment.');
    end if;
  else
    if invoice_row.status <> 'issued' and p_amount > 0 then
      return jsonb_build_object('ok', false, 'message', 'Only an issued invoice can receive a payment.');
    end if;
    insert into public.invoice_payments(id, invoice_id, amount, paid_on, method, reference, note, recorded_by)
    values(p_payment_id, p_invoice_id, p_amount, coalesce(p_paid_on, (now() at time zone 'Asia/Jakarta')::date),
      p_method, p_reference, p_note, p_staff_id);
  end if;
  select coalesce(sum(amount), 0) into paid_amount from public.invoice_payments where invoice_id = p_invoice_id;
  return jsonb_build_object('ok', true, 'paid', paid_amount,
    'outstanding', coalesce(invoice_row.amount_due, invoice_row.total) - paid_amount);
end $$;

grant execute on function public.record_invoice_payment(uuid, uuid, numeric, date, text, text, text, uuid) to anon, authenticated;

create or replace function public.complete_billed_reservation(
  p_reservation_id uuid, p_extra_spend numeric, p_expected_base numeric,
...
declare
  ...
  base_amount numeric;
  total_spend numeric;
  extra_amount numeric := coalesce(p_extra_spend, 0);
begin
  if extra_amount < 0 or extra_amount::text in ('NaN', 'Infinity', '-Infinity') then
    return jsonb_build_object('ok', false, 'message', 'Additional spending must be zero or more.');
  end if;
  select * into reservation_row from public.reservations where id = p_reservation_id for update;
  if not found or reservation_row.deleted_at is not null or reservation_row.status in ('Deleted', 'Cancelled', 'Cancelled (No Show)', 'No Show') then
    return jsonb_build_object('ok', false, 'message', 'This reservation cannot be completed.');
  end if;
  select greatest(coalesce(settlement_total, paid_total, 0), 0) into base_amount
    from public.reservation_money where reservation_id = p_reservation_id;
  if coalesce(base_amount, 0) <= 0 or p_expected_base is distinct from base_amount then
    return jsonb_build_object('ok', false, 'message', 'The billing amount changed. Reopen completion to review it.');
  end if;
  -- The parent lock serializes two completion clicks, even before a visit exists.
  if (select count(*) from public.visits where reservation_id = p_reservation_id and voided_at is null) > 1 then
    return jsonb_build_object('ok', false, 'message', 'This reservation has multiple visits. Review them before completing.');
  end if;
  select * into visit_row from public.visits
    where reservation_id = p_reservation_id and voided_at is null for update;
  total_spend := base_amount + extra_amount;
  if visit_row.id is null then
    insert into public.visits(guest_id, reservation_id, visit_type, visit_date, visit_time,
      pax, assigned_area, table_id, table_ids, spend_amount, billing_base_amount, extra_spend_amount,
      status, completed_at, notes, created_by)
    values(reservation_row.guest_id, p_reservation_id, 'Reservation', reservation_row.reservation_date,
      (now() at time zone 'Asia/Jakarta')::time, reservation_row.pax,
      reservation_row.assigned_area, reservation_row.table_id, reservation_row.table_ids,
      total_spend, base_amount, extra_amount, 'Done', now(), p_notes, p_staff_id)
    returning * into visit_row;
  else
    update public.visits set spend_amount = total_spend, billing_base_amount = base_amount,
      extra_spend_amount = extra_amount, status = 'Done', completed_at = coalesce(completed_at, now()),
      notes = coalesce(p_notes, notes), updated_at = now()
    where id = visit_row.id returning * into visit_row;
  end if;
  update public.reservations set status = 'Completed', updated_at = now() where id = p_reservation_id;
  return jsonb_build_object('ok', true, 'visit_id', visit_row.id, 'guest_id', visit_row.guest_id,
    'spend_amount', total_spend, 'base_amount', base_amount, 'extra_spend_amount', extra_amount);
end $$;

grant execute on function public.complete_billed_reservation(uuid, numeric, numeric, text, uuid) to anon, authenticated;
notify pgrst, 'reload schema';
commit;
