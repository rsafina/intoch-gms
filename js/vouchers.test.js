    ...
    const q = {
      ...
      _rows() {
        ...
        if (table === "wa_templates")
          return [{ key: "standalone_voucher", body: "Halo {nama}, Nilai: {nominal}, Kode: {kode}, {resto}, berlaku {berlaku}" }];
        return store.rows;
      },
      _match() {
        let rows = q
...
w.restaurantName = () => "Regression Restaurant";
w.eval(read("js/wa.js"));
...
(async () => {
  ...
  check("message renders the actual value", w.__lastWa.message.includes(w.vchValueText(withPhone)), "true");
