...
async function loadOwnerDashboard(){
  ...
  try{
    const features=typeof financialTrackingSettings==='function'?financialTrackingSettings():{depositEnabled:true,spendingEnabled:true};
    const [visits,reservations,overdue]=await Promise.all([
      ...
      features.depositEnabled?odRows('reservations','id,booking_name,guests(name),reservation_date,pax,deposit_due_at',q=>q.is('deleted_at',null).eq('deposit_required',true).in('status',['Incoming','Waitlist','Reserved']).lt('deposit_due_at',now.toISOString())):Promise.resolve([])
    ]);
...
function odRenderOverview(root,data){
 ...
 const features=typeof financialTrackingSettings==='function'?financialTrackingSettings():{depositEnabled:true,spendingEnabled:true};
 const spendCard=features.spendingEnabled?card(odText('Recorded spend','Belanja tercatat'),odMoney(m.spend),`${m.unrecorded} ${odText('visits without spend recorded','kunjungan belum mencatat belanja')}`):card(odText('Spending Tracking','Pelacakan Pengeluaran'),odText('Disabled','Dinonaktifkan'),odText('Missing spending is not counted as zero','Pengeluaran yang tidak tercatat tidak dianggap nol'));
 const depositDetail=features.depositEnabled?` · ${totals.incoming} ${odText('awaiting deposit','menunggu deposit')}`:'';
 const depositAlert=features.depositEnabled&&due.length?`<button class="od-alert" onclick="odShowDetails('deposits')"><span><strong>${due.length} ${odText('overdue deposits','deposit lewat jatuh tempo')}</strong><small>${odMoney(due.reduce((s,r)=>s+r.outstanding,0))} · ${odText('all outstanding dates','semua tanggal tertunggak')}</small></span>${odChevron}</button>`:'';
 ...
 <div class="od-metrics">${spendCard}${card(odText('Guest visits','Kunjungan tamu'),m.pax+' pax',`${m.current.length} ${odText('visits','kunjungan')}`)}${card(odText('Reservations','Reservasi'),totals.count,`${totals.pax} pax ${odText('confirmed','dikonfirmasi')}${depositDetail}`)}${card(odText('Walk-ins','Walk-in'),m.walkins,`${m.walkinPax} pax`)}</div>
 ...
 ${features.spendingEnabled?`<section class="od-card"><h2>${odText('Guest leaderboard','Peringkat tamu')}</h2><p class="od-subtitle">${odText('Recorded spend · selected period','Belanja tercatat · periode terpilih')}</p>${leaders.length?leaders.map(([id,amount],i)=>`<div class="od-leader"><span class="od-rank">${i+1}</span><span>${odEscape(names.find(n=>n.id===id)?.name||odText('Guest','Tamu'))}</span><strong>${odMoney(amount)}</strong></div>`).join(''):`<p class="od-empty">${odText('No recorded spending yet.','Belum ada belanja tercatat.')}</p>`}</section>`:''}
