...
(async () => {
 ...
 await db.exec('create table visits(visit_date date,status text,voided_at timestamptz,reservation_id uuid,table_id uuid,table_ids uuid[])');
 const picker=fs.readFileSync('migrations/20260913_table_picker_availability.sql','utf8');
 await db.exec(picker);await db.exec(picker);
 ...
 let event=await add();
 const availability=async(time='13:00',buffer=0,exclusive=false,exclude=null)=>(await q('select * from reservation_table_availability($1,$2,null,180,$3,$4,$5)',[date,time,buffer,exclusive,exclude])).rows;
 const isOccupied=(rows,id)=>rows.find(r=>r.table_id===id).occupied;
 assert.equal(isOccupied(await availability(),tables[0].id),true,'future booking disables its selected tables');
 assert.equal(isOccupied(await availability(),tables[12].id),false,'other tables remain available');
 assert.equal(isOccupied(await availability('16:00'),tables[0].id),false,'touching end does not overlap');
 assert.equal(isOccupied(await availability('16:00',30),tables[0].id),true,'preparation buffer overlaps existing visit');
 assert.equal(isOccupied(await availability('13:00',0,false,event),tables[0].id),false,'own reservation is excluded');
 assert.equal(isOccupied(await availability('13:00',0,true),tables[12].id),true,'whole-area request conflicts with any held table');
