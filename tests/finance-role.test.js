...
(async()=>{
 ...
 w.applyRoleToNav();
 assert.equal(w.document.querySelector('[data-nav="walkins"]').style.display,'none');
 const app=fs.readFileSync('js/app.js','utf8');
 const start=app.indexOf('async function initializeApplication(landingPage) {');
 assert.ok(start >= 0, 'initializer must be found');
 ...
 assert.ok(w.document.querySelector('[data-status="all"]').classList.contains('btn-primary'));
 assert.equal(w.document.querySelector('[data-status="deposits"]').classList.contains('btn-primary'),false);
 w.close();
